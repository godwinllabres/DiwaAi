...
class HybridChatbot:
    ...
    def __init__(
        ...
        intents_db_path: str = None,
        intents_json_path: str = "data/cavsu_intents.json"
    ):
        """
        ...
        Args:
            ...
            responses_path: Path to responses JSON (fallback)
            intents_db_path: SQLite path for intent storage
            intents_json_path: JSON fallback path for intent data
        """
        print("\n" + "=" * 60)
        ...
        print("\n[3/4] Loading intent metadata...")
        self.intents = load_intents(
            json_path=intents_json_path,
            db_path=intents_db_path or "data/cavsu_intents.db"
        )
        self.intent_metadata = {
            intent["tag"]: {
                ...
                "active": intent.get("active", True)
            }
            for intent in self.intents
        }
        self.responses_map = build_responses_map(self.intents)
        self.total_intents = len(self.responses_map)
        self.total_patterns = sum(len(intent.get("patterns", [])) for intent in self.intents)

        if self.responses_map:
            print(f"[OK] Loaded {len(self.responses_map)} intent responses")
        elif responses_path:
            with open(responses_path, "r", encoding="utf-8") as f:
                self.responses_map = json.load(f)
            self.total_intents = len(self.responses_map)
            self.total_patterns = sum(len(v) for v in self.responses_map.values())
            print(f"[OK] Loaded {len(self.responses_map)} intent responses from JSON fallback")
        else:
            raise ValueError("No intent source available. Provide intents_db_path or responses_path.")
    ...
    def get_all_intents(self) -> list:
        ...
        return sorted(self.responses_map)

    def get_intent_details(self, intent_tag: str) -> Optional[Dict]:
        """Return metadata for a specific intent."""
        if intent_tag not in self.responses_map:
            return None

        metadata = self.intent_metadata.get(intent_tag, {})
        return {
            "tag": intent_tag,
            "description": metadata.get("description", ""),
            "active": metadata.get("active", True),
            "pattern_count": len(metadata.get("patterns", [])),
            "response_count": len(self.responses_map[intent_tag]),
            "sample_patterns": metadata.get("patterns", [])[:10],
    ...
    def model_name(self) -> str:
        """Model name"""
        return "Hierarchical Hybrid Chatbot (NB + NN)"
...
class NeuralNetworkTrainer:
    ...
    def train(
        ...
        print("\n[1/5] Loading intents...")
        if os.path.exists(intents_db_path):
            print(f"[OK] Loading intents from SQLite DB: {intents_db_path}")
            intents = load_intents(json_path=intents_path, db_path=intents_db_path)
        else:
            print(f"[OK] Loading intents from JSON: {intents_path}")
        ...
        print(f"[OK] Loaded {len(patterns)} patterns from {len(intents)} intents")
