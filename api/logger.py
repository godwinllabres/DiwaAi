...
_PG_SCHEMA = [
    ...
    """,
    """
    CREATE TABLE IF NOT EXISTS tier_disagreements (
        id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        timestamp TEXT NOT NULL,
        query TEXT NOT NULL,
        nb_intent TEXT,
        nb_confidence DOUBLE PRECISION,
        nb_margin DOUBLE PRECISION,
        nn_intent TEXT,
        nn_confidence DOUBLE PRECISION,
        retrieval_topk TEXT,
...
_INDEX_DDL = [
    ...
    "CREATE INDEX IF NOT EXISTS idx_tier_disagreements_timestamp ON tier_disagreements (timestamp)",
]
...
class ChatLogger:
    ...
    def _init_sqlite(self):
        ...
        # Cross-tier disagreement queue (P1-5 arbitration) — one row per turn
        # where the NB tier was confident but the margin or the retrieval
        # agreement check escalated it, or the NN would have overridden NB.
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS tier_disagreements (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                timestamp TEXT NOT NULL,
    ...
    # ===== Tier disagreements (P1-5 arbitration review queue) =====

    def log_tier_disagreement(
        self,
        query: str,
        nb_intent: Optional[str],
        nb_confidence: float,
        nb_margin: float,
        reason: str,
        nn_intent: Optional[str] = None,
        nn_confidence: Optional[float] = None,
        retrieval_topk: Optional[List[Dict]] = None,
    ) -> None:
        """Record one cross-tier disagreement — the weekly review queue.

        Written from the chat path, so strictly best-effort: an audit-row
        failure must never fail a student's turn.
        """
        try:
            with self.lock:
                conn, cursor = self._connect()
                cursor.execute(
                    self._sql("""
                        INSERT INTO tier_disagreements
                            (timestamp, query, nb_intent, nb_confidence, nb_margin,
                             nn_intent, nn_confidence, retrieval_topk, reason)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """),
                    (
                        ...
                        float(nb_margin),
                        nn_intent,
                        float(nn_confidence) if nn_confidence is not None else None,
                        json.dumps(retrieval_topk or [], ensure_ascii=False),
                        reason,
                    ),
                )
                conn.commit()
            ...
            print(f"[ERROR] Failed to log tier disagreement: {e}")

    def get_tier_disagreements(self, days: int = 7, limit: int = 500) -> List[Dict]:
        """Recent tier disagreements, newest first."""
        try:
            ...
            cursor.execute(
                self._sql(
                    "SELECT * FROM tier_disagreements "
            ...
            for row in rows:
                try:
                    row["retrieval_topk"] = json.loads(row.get("retrieval_topk") or "[]")
                except (TypeError, ValueError):
                    pass
            return rows
        except Exception as e:
            print(f"[ERROR] Failed to read tier disagreements: {e}")
            return []
